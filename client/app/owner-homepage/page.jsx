...
export default function OwnerDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadStats() {
      try {
        const res = await fetch(
          `/api/owner-stats?owner_id=${encodeURIComponent(ownerId)}`,
        );
    ...
    async function loadRentals() {
      try {
        const res = await fetch(
          `/api/owner-rentals?owner_id=${encodeURIComponent(ownerId)}`,
        );
        const data = await res.json();
        if (data.success) {
      ...
      <header className="bg-[url('/owner-dashboard-image.jpg')] bg-cover bg-center h-[500px] md:h-[600px] xl:h-[766px]">
        <div className="flex flex-col items-center justify-center gap-9 md:gap-6 text-center px-4 h-full">
          ...
          <button
            ...
            className="bg-red-600 hover:bg-red-700 text-white font-semibold px-6 py-3 md:px-8 md:py-4 rounded-full cursor-pointer text-sm md:text-base xl:text-lg transition-colors whitespace-nowrap"
          >
      ...
      <div className="rentals-container">
              ...
              <tbody>
                {rentals.length > 0 ? (
                  rentals.map((rental) => (
                    ...
                    >
                      <td className="py-4 px-4 text-center text-sky-800">
                        {rental.product_name}
                      </td>
                  ...
                  <tr>
                    <td
                      colSpan="5"
                      className="py-8 px-4 text-center text-gray-600"
                    >
        ...
        <div className="info-card-container flex flex-col lg:flex-row bg-zinc-800 justify-start items-center gap-6 md:gap-8 lg:gap-20 xl:gap-36 px-4 md:px-8 lg:px-20 xl:px-36 py-10 md:py-12 lg:py-14">
          <div className="text text-white text-center lg:text-left">
            <h1 className="max-w-xs md:max-w-md lg:max-w-lg text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold">
              Tips & Insights
            </h1>
          </div>
